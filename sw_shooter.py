...
from score_board import ScoreBoard as SB
...
class Shooter():
    ...
    def _check_keydown_events(self,event):
        """respond to the keys pressed by the player"""
    ...
    def _update_aliens(self):
        ...
        for alien in self.aliens.sprites():
            alien.rect.y+=(
                self.settings.aliens_speed * self.settings.aliens_direction)
        self._check_aliens_edges()
        
        if pg.sprite.spritecollideany(self.ship,self.aliens):
            self._ship_hit()
        self._check_aliens_disappearing()
    ...
    def _fire_bullet(self):
        ...
        if len(self.bullets) < self.settings.bullets_limit:
            ...
            fire=pg.mixer.Sound("sounds/fire.wav")
            fire.play()
    ...
    def _check_aliens_disappearing(self):
        """respond when an alien reaches the the left edge of the screen"""
        for alien in self.aliens.sprites():
    ...
    def _update_screen(self):
        ...
        milliseconds=(pg.time.get_ticks()-self.start_time)
        if milliseconds>90:
    ...
    def _update_bullets(self):
        ...
        for aliens in collisions.values():
            self.settings.score+=self.settings.alien_points * len(aliens)
            self.sb.prep_score()
            self.sb.check_high_score()
            pg.mixer.Sound("sounds/explosion.flac").play()
            self._make_explosion(aliens)
        if not self.aliens.sprites():
            self._start_new_level()

    def _make_explosion(self,aliens):
        """make an explosion"""
        for alien in aliens:
